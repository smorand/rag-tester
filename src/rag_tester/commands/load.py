...
def load_command(
    ...
    database: str = typer.Option(..., "--database", "-d", help="Database connection string (e.g., chromadb://localhost:8000/collection)"),
    embedding: str = typer.Option(..., "--embedding", "-e", help="Embedding model identifier (e.g., sentence-transformers/all-MiniLM-L6-v2)"),
    ...
    exit_code = asyncio.run(_load_async(
        file=file,
        database=database,
        embedding=embedding,
        mode=mode,
...
async def _load_async(
    ...
    try:
        ...
        if mode != "initial":
            error_console.print(f"[red]Error: Mode '{mode}' is not yet implemented. Only 'initial' mode is supported.[/red]")
            return 1
        
        # Parse database connection string
        # Format: chromadb://host:port/collection_name
        if not database.startswith("chromadb://"):
            error_console.print("[red]Error: Only ChromaDB is currently supported. Use chromadb://host:port/collection[/red]")
            return 1
            
        db_parts = database.replace("chromadb://", "").split("/")
        if len(db_parts) != 2:
            error_console.print("[red]Error: Invalid database connection string. Expected format: chromadb://host:port/collection[/red]")
            return 1
